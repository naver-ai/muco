...
class EvalCollator:
    ...
    def __call__(self, examples):
        ...
        inputs = process_vlm_inputs_fns[self.args.model_backbone](examples,
...
class CLIPCollator:
    ...
    def _get_batch_inputs(self, examples):
        ...
        for example in examples:
            ...
            if text:
                text_exist = True
            text_inputs = self.txt_processors(text, padding=getattr(self.args, "padding", True), max_length=self.args.max_len, truncation=True, return_tensors="pt")
...
class MultimodalEvalDataCollator:
                        ...
                        for image_idx in range(num_images):
                            ...
                            if bytes is None and path is None:
                                ...
                                if not os.path.exists(path):
                                    path = path.replace('/images/', '/image-tasks/')
                                image = Image.open(path)
                            else:
                                print_rank(f"\n{'=' * 50}\nsomething went wrong with a data point from {example['global_dataset_name']}, neither bytes or path is given. \n\t\tquery_text: {example['query_text']}")
                            if not self.args.resize_use_processor and image is not None and image_resolution:
                                image = image.resize(image_resolution)
                            if image is not None and (image_resolution is not None and self.args.image_decay_factor is not None):
                                assert image_resolution is None, "image_resolution is conflicting with image_decay_factor"
                                assert self.args.model_backbone == QWEN2_VL, "image_decay_factor is only supported for Qwen2-VL"
                                # TODO: this is a hacky way to decay image resolution, need to be refactored
                                max_pixels = max(self.args.resize_min_pixels, self.args.resize_max_pixels * self.args.image_decay_factor ** (num_images - image_idx))
                                width, height = image.size
                                resized_height, resized_width = smart_resize(
                                    height,
                                    width,
                                    min_pixels=self.args.resize_min_pixels,
    ...
    def __call__(self, examples):
        ...
        """
        process_fn = process_vlm_inputs_fns[self.args.model_backbone]
        ...
        processed_inputs = process_fn(inputs, processor=self.processor, max_length=self.args.max_len)
